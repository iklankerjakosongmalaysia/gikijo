      ...
      data.job_list.map((item) => {
        ...
        if (item.timestamp_active && item.timestamp_expired) {
          ...
          if (item.status_id === 3) {
            ...
            if (item.is_free == true) {
              jobTimeline[0].innerHTML = `Free ∙ Maximum of ${item.payment_info.max_active_post} published post at a time ∙ ${item.payment_info.day_visibility} days (${active_date} - ${expired_date})`;
            } else {
              jobTimeline[0].innerHTML = `Paid ∙ Maximum of ${item.payment_info.max_active_post} published post at a time ∙ ${item.payment_info.day_visibility} days (${active_date} - ${expired_date})`;
            }
          }
        } else {
        ...
        if (isPaid_isFree === false) {
          ...
          firstBtn.addEventListener("click", function () {
            alert(
              "Sorry, this job slot has expired. All posts related to this job slot are no longer available in our Telegram channels. Please create a new job slot to advertise a new job opportunity. Thank you!"
            );
          });
